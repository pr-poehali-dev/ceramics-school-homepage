...
def handler(event: dict, context) -> dict:
    '''
    Создаёт онлайн-платёж ЮKassa для уже сохранённого заказа (таблица orders).
    Args: event с httpMethod, body (JSON: number, amount, user_email, user_name, return_url, cart_items)
          context — объект с request_id
    Returns: HTTP-ответ со ссылкой на оплату (payment_url)
    '''
    # CORS preflight
    ...
    order_number = str(data.get('number') or '').strip()
    ...
    if not order_number:
        return {
            'statusCode': 400,
            'headers': HEADERS,
            'body': json.dumps({'error': 'number (order number) is required'})
        }
    ...
    try:
        ...
        cur.execute(f"""
            UPDATE {S}orders
            SET yookassa_payment_id = %s, payment_url = %s, status = 'pending', updated_at = %s
            WHERE id = %s
