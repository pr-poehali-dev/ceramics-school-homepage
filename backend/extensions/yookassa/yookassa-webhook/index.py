...
def handler(event: dict, context) -> dict:
    '''
    Принимает уведомления от ЮKassa об изменении статуса платежа и обновляет заказ в orders.
    Статус платежа перепроверяется напрямую через API ЮKassa (webhook не подписан).
    Args: event с httpMethod, body (JSON от ЮKassa: event, object.id, object.metadata)
          context — объект с request_id
    Returns: HTTP-ответ {'status': 'ok'}
    '''
