...
const SECTIONS = [
  { id: 'delivery', label: 'Отправка в Москву', icon: 'Truck' },
  { id: 'pickup', label: 'Условия выдачи изделия', icon: 'PackageCheck' },
];

const SuzdalInfo = () => {
  const c = usePageContent('suzdal-info');
  usePageMeta({
    title: c.metaTitle,
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <section id="delivery" className="mt-12 scroll-mt-24 rounded-2xl border border-border bg-card p-6 md:p-8">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-display text-2xl font-semibold">Отправка в Москву</h2>
            </div>
            <div className="mt-5 space-y-3 text-sm leading-relaxed text-muted-foreground">
              ...
              <div className="rounded-xl bg-secondary/40 p-4">
                <p className="flex gap-2">
                  <Icon name="Home" size={16} className="mt-0.5 shrink-0 text-primary" />
                  <span>{c.localPickupNote}</span>
                </p>
              </div>
            </div>
          </section>
